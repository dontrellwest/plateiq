...
## The rest-end alert (2026-09-05, after the first gym session)

The owner's report was "the buzz at the end of the rest timer is too easy to miss and has no sound
with it". A louder buzz alone does not fix that, because iOS suspends the app when the phone is in
a pocket and nothing in-app can fire at all. So the alert is now three layers:

- **A pattern you can feel.** One `Success` da-dum, then two three-pulse bursts, the
  ring-then-ring-again cadence of an incoming call. It runs for 1.4 s and stops for good, so it can
  never nag. Pulses are spaced at least 110 ms apart because the Taptic Engine coalesces anything
  tighter.
- **A chime.** `assets/rest-done.wav`, a rising perfect fifth, generated by `scripts/make-chime.js`
  from arithmetic so the project owns it outright and no third-party audio licence is in play at
  App Store review. The audio session uses `playsInSilentMode` with `mixWithOthers`, so it is
  audible with the ring switch on silent and never interrupts the user's music. On by default, and
  the Settings row says both of those things out loud so nobody is startled in a quiet room.
- **A lock-screen alert.** A local notification scheduled for the rest's existing wall-clock end
  time, which is the only thing that reaches a pocketed phone. Off by default: turning it on is
  what triggers the iOS permission prompt, and an app that asks before the user has done anything
  gets denied. If the user declines, the switch turns itself back off rather than lying.

`src/platform/restAlert.ts` is the single place that decides what fires and what is armed. It
