...
// prototype's green run. Counts: PLATEIQ_FULL=1 runs the prototype's 1,000,000 scenarios and
// 5,000 users × 50 actions; otherwise a reduced prefix (PLATEIQ_SCENARIOS / PLATEIQ_USERS).
...
const A_TOTAL = FULL ? 1000000 : Number(process.env.PLATEIQ_SCENARIOS || 20000);
const B_USERS = FULL ? 5000 : Number(process.env.PLATEIQ_USERS || 200);
const B_ACTIONS = 50;

const RESULTS = { sets: 0, actions: 0, failures: [] as string[], categories: {} as Record<string, number> };
...
function randomize(c: PlateIQLogic, rnd: () => number) {
...
function phaseA() {
  ...
  const inst = fresh();
  for (let aDone = 0; aDone < A_TOTAL; aDone++) {
    randomize(inst, rndA);
...
function checkState(c: PlateIQLogic, tag: string) {
  const s = c.state;
...
describe('PlateIQ stress harness (' + (FULL ? 'FULL' : 'reduced') + ')', () => {
  test('A · ' + A_TOTAL.toLocaleString() + ' solver scenarios: accounting, sleeve fit, inventory, landmine bound, alt fixed points', () => {
    ...
    console.log('phase A: ' + A_TOTAL.toLocaleString() + ' scenarios, ' + RESULTS.sets.toLocaleString() + ' sets checked; ' + (Object.keys(RESULTS.categories).length ? report() : 'no failures'));
    expect(report()).toBe('');
  }, 24 * 3600 * 1000);

  test('B · ' + (B_USERS * B_ACTIONS).toLocaleString() + ' UI actions across ' + B_USERS.toLocaleString() + ' users: state-machine + tour invariants', () => {
    const before = Object.keys(RESULTS.categories).length;
    phaseB();
    // eslint-disable-next-line no-console
    console.log('phase B: ' + RESULTS.actions.toLocaleString() + ' actions; ' + (Object.keys(RESULTS.categories).length > before ? report() : 'no failures'));
    expect(report()).toBe('');
  }, 24 * 3600 * 1000);
});
